...
export default async function ApplicationDetailPage({
  ...
  return withAppStore(auth.store, async () => {
    const detail = await getApplicationDetail(auth.candidateId, id);

    if (!detail) {
      notFound();
    }

    let workflow: Awaited<ReturnType<typeof getApplicationWorkflow>> | null =
      null;
    let workflowError = '';
      ...
      try {
        ...
        workflowError =
          error instanceof Error
            ? error.message
            : 'Application workflow is not available.';
      }
    }
    ...
    return (
      <div className="page-grid">
        <SectionCard
          actions={
            <Link className="ghost-link" href="/applications">
              Back to board
            </Link>
          }
          ...
          title={
            detail.job?.title ??
            detail.attempt.metadata.title?.toString() ??
            'Application detail'
          }
        >
          <div className="detail-grid">
            <div>
              ...
              <strong>
                {detail.job?.company ??
                  detail.attempt.metadata.company?.toString() ??
                  'Unknown'}
              </strong>
            </div>
            <div>
