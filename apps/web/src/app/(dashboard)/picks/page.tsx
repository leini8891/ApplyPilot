...
import { getDailyPicks, withAppStore } from '@/server/services/app-service';
...
export default async function DailyPicksPage() {
  ...
  return withAppStore(auth.store, async () => {
    const dailyPicks = await getDailyPicks(auth.candidateId);
                ...
                <article className="pick-card" key={pick.job.id}>
                  <div className="pick-header">
                    ...
                    <StatusPill
                      ...
                      tone={scoreTone(pick.score.overall)}
                    />
                  </div>

                  <p className="pick-summary">
                    {pick.job.description ||
                      'No description captured yet for this role.'}
                  </p>
                  ...
                  <div className="pick-meta">
                    ...
                    <span className="muted-copy">
                      Refreshed {formatDateTime(dailyPicks.generatedAt)}
                    </span>
                  </div>
                    ...
                    <div className="signal-block">
                      ...
                      {pick.watchouts.length > 0 ? (
                        ...
                        <p className="muted-copy">
                          No major watchouts surfaced from the saved data.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {pick.knowledgeMatches.length > 0 ? (
                    <div className="prep-match-list">
                      ...
                      {pick.resumeMatches.map((match) => (
                        <article
                          className="prep-match-row"
                          key={`${match.sourceLabel}-${match.title}`}
                        >
                          <div>
                  ...
                  <div className="pick-footer">
                    <Link
                      ...
                      href={
                        pick.applicationId
                          ? `/applications/${pick.applicationId}`
                          : '/applications'
                      }
                    >
                      Prepare application
