...
export default async function HomePage() {
  ...
  return withAppStore(auth.store, async () => {
    const [snapshot, dailyPicks] = await Promise.all([
      getDashboardData(auth.candidateId),
      getDailyPicks(auth.candidateId),
    ]);
        ...
        <div className="page-grid">
          <section className="stats-row">
            ...
            <StatCard
              label="Top match"
              tone="success"
              value={topPick ? `${topPick.score.overall}%` : 'N/A'}
            />
          </section>
          ...
          >
            <div className="checklist">
              <p>
                1. Parse a master resume into a structured candidate profile.
              </p>
              <p>2. Save keywords, regions, salary floor, and VIP companies.</p>
              <p>
                3. Review the top three roles, then apply manually with more
                confidence.
              </p>
            </div>
          </SectionCard>
          ...
          <div className="two-column-grid">
            <SectionCard
              actions={
                <Link className="ghost-link" href="/picks">
                  Open all picks
                </Link>
              }
                ...
                {dailyPicks.picks.length > 0 ? (
                  ...
                  <article className="list-row">
                    <div>
                      ...
                      <p>
                        Upload a resume and save preferences first, then
                        ApplyPilot can rank roles for you.
                      </p>
                    </div>
                  </article>
                )}
            ...
            >
              <p className="muted-copy">
                {snapshot.profile?.summary ?? 'No parsed profile yet.'}
              </p>
          ...
          <SectionCard
            actions={
              <Link className="ghost-link" href="/runs">
                Open runs
              </Link>
            }
